...
After each run where **val_bpb improved** (checkpoint.pt exists), create ALL supporting files:
...
2. Create experiment directory and move checkpoint:
   ...
   ```
3. Run inference to generate sample:
   ```bash
   uv run inference.py \
       --checkpoint "$exp_dir/checkpoint.pt" \
       --prompt "Once upon a time" \
       ...
       --output "$exp_dir/sample_output.txt"
   ```
4. Create git_log.txt: `git log -1 > "$exp_dir/git_log.txt"`
5. Copy results.tsv: `cp results.tsv "$exp_dir/results.tsv.global"`
6. Create model_info.txt with experiment metadata
7. Verify ALL files exist:
   ```bash
   ls "$exp_dir/checkpoint.pt" "$exp_dir/git_log.txt" "$exp_dir/results.tsv.global" "$exp_dir/sample_output.txt" "$exp_dir/model_info.txt"
   ```
